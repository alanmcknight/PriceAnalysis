...
server = function(input, output, session) {
    ...
    my_data3 <- read.csv("ApricotQuotesMaster3.csv", header = TRUE, stringsAsFactors =FALSE, fileEncoding="latin1")
...
#    pos <- positives()
  ...
  my_data6 <- reactive({
      data6 <- data()
      data6 <- subset(data6[,c(1:(which(colnames(data6)=="Site.Name.1")), ncol(data6))], data6$Apricot.Position <4)
      data6
  })
...
#    my_data7$Adjusted.Price.Range <- 10
  ...
  data10 <- reactive({
    my_data10 <- data()
    my_data10 <- my_data10[!is.na(my_data10$SYSTEM.NAME),]
  ...
  data11 <- reactive({
    ...
    Summary$Average.Difference.To.2nd <- round(Summary$Sum.of.Difference/Summary$Position1s,digits=2)
      ...
      colnames(Summary)[4:6] <- c("Position1s", "Sum.of.Difference", "Mamimum")
      Summary$Average.Difference.To.2nd <- round(Summary$Sum.of.Difference/Summary$Position1s,digits=2)
    }
