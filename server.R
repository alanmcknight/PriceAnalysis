...
#    my_data7$Adjusted.Price.Range <- 10
  ...
  data8 <- reactive({
    ...
    for(j in 1:nrow(my_data8)){	
      if(((my_data8$Selected.Provider.Price[j] +as.numeric(input$integer2))*(1+((as.numeric(input$percentage2))/100))) - as.numeric(my_data8$Price.Position.1[j]) <= 0){
        my_data8[j, UploadColumnNumbers+1] = 1
      } else if ((((my_data8$Selected.Provider.Price[j] +as.numeric(input$integer2)))*(1+((as.numeric(input$percentage2))/100))) - as.numeric(my_data8$Price.Position.2[j]) <= 0){
        my_data8[j, UploadColumnNumbers+1] = 2
      } else if ((((my_data8$Selected.Provider.Price[j] +as.numeric(input$integer2)))*(1+((as.numeric(input$percentage2))/100))) - as.numeric(my_data8$Price.Position.3[j]) <= 0){
    ...
    #colnames(ProviderGroup)<- c("Apricot Agg (OGI standalone)","Position 1","Position 2", "Position 3", "Position 4+")
    # colnames(ProviderGroup)[1] <- "Apricot Agg (OGI standalone)"
    # colnames(ProviderGroup)[3] <- ""
    # colnames(ProviderGroup)[3] <- "Position 2"
    # colnames(ProviderGroup)[4] <- "Position 3"
  ...
  data10 <- reactive({
    ...
    my_data10[, UploadColumnNumbers+1] <- (as.numeric(my_data10[, (UploadColumnNumbers-2)])+as.numeric(input$integer))*(as.numeric(input$percentage)/100)
    ...
    if(input$integer <= 0){
      ...
      my_data10[, UploadColumnNumbers+2] <- (as.numeric(my_data10[, UploadColumnNumbers+1]) - as.numeric(my_data10[,Position2]))*(as.numeric(input$percentage)/100)
      my_data10[, UploadColumnNumbers+3] <- ifelse(my_data10[, UploadColumnNumbers+2] <= 0, 1, 0)
      colnames(my_data10)[UploadColumnNumbers+3] <- "Adjusted.Position.1"
      filterSummary <- count(my_data10, c("Site.Name.1", c(input$filterName, "Adjusted.Position.1")))
      filterSummary3 <- filterSummary[ which(filterSummary$Adjusted.Position.1 == 1),]
